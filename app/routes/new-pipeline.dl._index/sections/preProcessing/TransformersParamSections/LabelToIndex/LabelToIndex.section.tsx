...
import { LabelNamesField } from "./LabelNames.field"


/**
 * delegated props are not applied
 */
export function LabelToIndexSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("transformersData.LabelToIndex", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.LabelToIndex")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of Label to index</DialogTitle>
