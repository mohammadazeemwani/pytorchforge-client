 ...
 */
export function ToTensorSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("transformersData.ToTensor", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.ToTensor")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of ToTensor</DialogTitle>
