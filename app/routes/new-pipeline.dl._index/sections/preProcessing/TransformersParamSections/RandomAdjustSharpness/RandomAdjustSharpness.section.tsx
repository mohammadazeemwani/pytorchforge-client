...
import { SharpnessFactorField } from "./SharpnessFactor.field"
import { PField } from "./P.field"

/**
 * delegated props are not applied
 */
export function RandomAdjustSharpnessSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("transformersData.RandomAdjustSharpness", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.RandomAdjustSharpness")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of Random Adjustment Sharpness</DialogTitle>
        ...
        <div className="space-y-4 sm:space-y-6" key={resetKey}>
          <SharpnessFactorField form={form} />
          <PField form={form} />
        </div>
