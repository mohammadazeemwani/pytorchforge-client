...
import { NmelsField } from "./Nmels.field"
import { FminField } from "./Fmin.field"
import { FmaxField } from "./Fmax.field"
import { HopLengthField } from "./HopLength.field"

/**
 * delegated props are not applied
 */
export function MelSpectrogramSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("transformersData.MelSpectrogram", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.MelSpectrogram")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of MelSpectrogram</DialogTitle>
