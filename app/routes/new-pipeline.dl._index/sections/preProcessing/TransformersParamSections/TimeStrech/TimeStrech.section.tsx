...
import { FixedRateField } from "./FixedRate.field"

/**
 * delegated props are not applied
 */
export function TimeStrechSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("transformersData.TimeStrech", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.TimeStrech")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of TimeStrech</DialogTitle>
