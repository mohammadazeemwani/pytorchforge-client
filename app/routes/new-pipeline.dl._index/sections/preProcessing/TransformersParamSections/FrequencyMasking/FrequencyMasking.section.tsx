...
import { FreqMaskParamField } from "./FreqMaskParam.field"
import { IIDMasksField } from "./IIDMasks.field"


/**
 * delegated props are not applied
 */
export function FrequencyMaskingSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("transformersData.FrequencyMasking", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.FrequencyMasking")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of FrequencyMasking</DialogTitle>
        ...
        <div className="space-y-4 sm:space-y-6" key={resetKey}>
          <FreqMaskParamField form={form} />
          <IIDMasksField form={form} />
        </div>
