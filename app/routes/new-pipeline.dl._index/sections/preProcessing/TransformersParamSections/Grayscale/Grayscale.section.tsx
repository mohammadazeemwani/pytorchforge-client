...
import { NumOutputChannelsField } from "./NumOutputChannels.field"
...
export function GrayscaleSection({
  ...
  const { error } = form.getFieldState(
    "transformersData.Grayscale",
    form.formState,
  )

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.Grayscale")
    setResetKey((k) => k + 1)
  }, [form])
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="mt-0">Params of Grayscale</DialogTitle>
        ...
        <div className="space-y-4 sm:space-y-6" key={resetKey}>
          <NumOutputChannelsField form={form} />
        </div>
