...
import { SampleRateField } from "./SampleRate.field"
import { NmfccField } from "./Nmfcc.field"
import { DctTypeField } from "./DctType.field"
import { LogMelsField } from "./LogMels.field"

/**
 * delegated props are not applied
 */
export function MFCCSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("transformersData.MFCC", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.MFCC")
    setResetKey((k) => k + 1)
  }, [form])
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="mt-0">Params of MFCC</DialogTitle>
        ...
        <div className="space-y-4 sm:space-y-6" key={resetKey}>
          <SampleRateField form={form} />
          <NmfccField form={form} />
          <DctTypeField form={form} />
          <LogMelsField form={form} />
        </div>
