...
import { PadIfNeededField } from "./PadIfNeeded.field"

export function RandomCropSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("transformersData.RandomCrop", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.RandomCrop")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of RandomCrop</DialogTitle>
