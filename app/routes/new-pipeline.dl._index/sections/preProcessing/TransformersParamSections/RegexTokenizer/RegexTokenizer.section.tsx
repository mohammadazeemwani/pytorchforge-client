...
import { PatternsListField } from "./PatternsList.field"

/**
 * delegated props are not applied
 */
export function RegexTokenizerSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("transformersData.RegexTokenizer", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.RegexTokenizer")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of RegexTokenizer</DialogTitle>
