...
import { BeginField } from "./Begin.field"


/**
 * delegated props are not applied
 */
export function AddTokenSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("transformersData.AddToken", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.AddToken")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of Add token</DialogTitle>
