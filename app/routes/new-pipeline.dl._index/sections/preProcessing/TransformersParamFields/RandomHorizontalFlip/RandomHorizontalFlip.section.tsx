 ...
 */
export function RandomHorizontalFlipSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("transformersData.RandomHorizontalFlip", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.RandomHorizontalFlip")
    setResetKey(k => k+1)
  }, [form])
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">
            Params of Random Horizontal Flip
          </DialogTitle>
