...
import { TokenizerField } from "./Tokenizer.field"

/**
 * delegated props are not applied
 */
export function BERTTokenizerSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("transformersData.BERTTokenizer", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.BERTTokenizer")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of BERTTokenizer</DialogTitle>
