...
import { TimeMaskParamField } from "./TimeMaskParam.field"
 ...
 */
export function TimeMaskingSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("transformersData.TimeMasking", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.TimeMasking")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of TimeMasking</DialogTitle>
