 ...
 */
export function ConvertImageDtypeSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("transformersData.ConvertImageDtype", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("transformersData.ConvertImageDtype")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of ConvertImageDtype</DialogTitle>
