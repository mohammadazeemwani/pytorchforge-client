...
import { DilationField } from "./Dilation.field"
import { CeilModeField } from "./CeilMode.field"
import { CountIncludePadField } from "./CountIncludePad.field"

/**
 * delegated props are not applied
 */
export function AvgPool3dSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("customModelsData.AvgPool3d", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("customModelsData.AvgPool3d")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of AvgPool3d</DialogTitle>
        ...
        <div className="space-y-4 sm:space-y-6" key={resetKey}>
          ...
          <CeilModeField form={form} />
          <CountIncludePadField form={form} />
        </div>
