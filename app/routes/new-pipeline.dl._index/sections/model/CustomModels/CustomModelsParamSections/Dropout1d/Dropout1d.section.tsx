...
import { InplaceField } from "./Inplace.field"

/**
 * delegated props are not applied
 */
export function Dropout1dSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("customModelsData.Dropout1d", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("customModelsData.Dropout1d")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of Dropout1d</DialogTitle>
