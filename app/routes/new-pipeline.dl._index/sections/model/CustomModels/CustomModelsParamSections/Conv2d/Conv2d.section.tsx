...
import { PaddingModeField } from "./PaddingMode.field"

/**
 * delegated props are not applied
 */
export function Conv2dSection({
  className,
  form,
  ...delegated
  ...
  const { error } = form.getFieldState("customModelsData.Conv2d", form.formState)

  const resetSection = React.useCallback(() => {
    form.resetField("customModelsData.Conv2d")
    setResetKey(k => k+1)
  }, [form])
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle className="mt-0">Params of Conv2d</DialogTitle>
